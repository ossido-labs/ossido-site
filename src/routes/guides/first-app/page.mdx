...
`OssidoPage<'/'>` from the generated `ossido/types` module and the props are exactly
...
const PokemonPage: OssidoPage<'/pokemons/[pokemon]'> = ({ name, id, height, weight }) => (
  <>
    <title>{name}</title>
    <h1>
      {name} #{id}
...
You already used `Link` from `ossido` in the list. It keeps navigation client-side
