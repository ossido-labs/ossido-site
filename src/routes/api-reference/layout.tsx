...
import { ECOSYSTEM_LABEL, groupByKind, type Ecosystem } from '@/content/api-reference';
 ...
 */
export default function ApiReferenceLayout({ children }: React.PropsWithChildren) {
  ...
  const headings: Array<Heading> = entry
    ? [
        ...
        ...(entry.examples.length ? [{ id: 'examples', text: 'Examples', level: 2 }] : []),
      ]
  ...
  return (
    <ContentLayout sidebar={<ReferenceSidebar />} ariaLabel="API Reference" headings={headings}>
      {children}
    </ContentLayout>
  );
}
