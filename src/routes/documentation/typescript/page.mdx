...
```ts
declare module '@ossido-labs/ossido/types' {
...
```ts
import type { Pokemon } from '@ossido-labs/ossido/types';
```
...
Pages and layouts are typed by _path_, so a component's props always match its
