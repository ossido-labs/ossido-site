...
annotate the component with `OssidoPage` from the generated `ossido/types` module:
...
the component with `OssidoLayout<'/path'>` from `ossido/types` instead - it gives
you the handler's data **and** `children`, both typed:

```tsx
import type { OssidoLayout } from 'ossido/types';
...
Use `Link` from `ossido` for in-app navigation - it keeps you in the SPA (fetching
