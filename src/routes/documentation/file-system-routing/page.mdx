...
Routes live in `src/routes`. The folder structure _is_ the URL structure -
...
export default function Error({ reset }: OssidoErrorProps) {
  return <button onClick={reset}>Try again</button>;
}
```

Place one at the root to catch the whole app, or in any subtree to scope recovery
to that branch. See [Error Handling](/documentation/error-handling) for the props,
boundary behaviour, and the server-side reporting hook.
...
**Next:** [Ossido Application](/documentation/ossido-application) · Back to
