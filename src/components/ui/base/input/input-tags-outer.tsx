...
import { HintText } from '@/components/ui/base/input/hint-text.tsx';
import { InputBase } from '@/components/ui/base/input/input.tsx';
import { Label } from '@/components/ui/base/input/label.tsx';
import { Tag, TagGroup, TagList } from '@/components/ui/base/tags/tags.tsx';
...
export const InputTagsOuter = ({
  ...
  const [internalEntries, setInternalEntries] = useState<Array<TagEntry>>(() =>
    (defaultValue ?? []).map((label) => ({ id: nextId(), label })),
  );

  const prevControlledValue = useRef<Array<string>>([]);
  const controlledEntries = useRef<Array<TagEntry>>([]);
  ...
  const entries = (() => {
    ...
    const newEntries: Array<TagEntry> = [];
