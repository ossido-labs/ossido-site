...
const REST_QUAT = new THREE.Quaternion().setFromEuler(new THREE.Euler(ISO_X, ISO_Y, 0));
...
function useEngravedGeometry(): THREE.BufferGeometry | null {
  const [geometry, setGeometry] = React.useState<THREE.BufferGeometry | null>(engravedGeometry);
...
const SpinningCube: React.FC<SpinningCubeProps> = ({ color, duration, delay, onReady }) => {
  ...
  // Suspends until every map is decoded, so the spin-in only begins once the
  // textures are ready to show.
  const { map, normalMap, roughnessMap, metalnessMap, aoMap } = useTexture(TEXTURES);
  ...
  useFrame((_, rawDelta) => {
    ...
    follow.current.x = THREE.MathUtils.damp(follow.current.x, targetX, 5, delta);
    follow.current.y = THREE.MathUtils.damp(follow.current.y, targetY, 5, delta);
    ...
    if (progress < 1) {
      ...
      if (s.sinceStep >= STEP_INTERVAL) {
        k.step.setFromAxisAngle(STEP_AXES[s.axis % STEP_AXES.length], Math.PI / 2);
        s.axis += 1;
    ...
    if (glowUniform.current) {
      glow.current = clamp01(glow.current + (progress >= GLOW_START ? delta / GLOW_FADE : 0));
      glowUniform.current.value = EMISSIVE_INTENSITY * easeInOutCubic(glow.current);
    }
  });
  ...
  return (
    <mesh ref={attachMesh} geometry={geometry} material={material} castShadow receiveShadow />
  );
};
...
const CubeScene: React.FC<CubeSceneProps> = ({ color, duration, delay, onReady }) => (
  <>
    <ZoomToCanvas />
    ...
    />
    <directionalLight position={[-6, -3, -4]} intensity={0.15} color={LIGHT_FILL} />
    ...
    <Environment resolution={256} frames={1}>
      ...
      <Lightformer
        ...
        rotation={[0, -0.5, 0.15]}
      />
      <Lightformer form="rect" intensity={0.6} color="#9fb4d0" position={[-5, 2, 1]} scale={[6, 6, 1]} />
      <Lightformer form="rect" intensity={1.1} color="#ffd8b0" position={[0, -4, -3]} scale={[6, 3, 1]} />
    </Environment>
    <React.Suspense fallback={null}>
      <SpinningCube color={color} duration={duration} delay={delay} onReady={onReady} />
    </React.Suspense>
    <EffectComposer>
      {/* Only the emissive engravings clear this threshold (the obsidian is
          near-black), so the glow blooms while the surface stays crisp. */}
      <Bloom mipmapBlur intensity={0.4} radius={0.4} luminanceThreshold={0.4} luminanceSmoothing={0.2} />
      <SMAA />
    </EffectComposer>
  </>
);
  ...
  return (
    ...
    >
      {/* zoom is pixels-per-world-unit; sized so the ~3.46-unit diagonal of the
          rotating cube fits the canvas with margin at any `size`. */}
      <Canvas
        orthographic
        shadows
        dpr={[1, 2]}
        gl={{ alpha: true, antialias: true }}
        camera={{ position: [0, 0, 10], zoom: size / ZOOM_DIVISOR, near: 0.1, far: 100 }}
        style={{ width: '100%', height: '100%', display: 'block' }}
      >
        <CubeScene color={color} duration={duration} delay={delay} onReady={handleReady} />
      </Canvas>
    </div>
  );
}
