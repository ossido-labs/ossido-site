...
import { Toaster, toast } from '@/components/ui/base/toast/toast';
...
const meta = {
  ...
  tags: ['autodocs'],
  decorators: [(Story): ReactElement => <ToastCanvas><Story /></ToastCanvas>],
} satisfies Meta;
...
export const Default: Story = {
  render: (): ReactElement => (
    <Button onPress={() => toast('Deployment queued', { description: 'Your changes are on the way.' })}>
      Show toast
    </Button>
  ),
};
...
export const Types: Story = {
  render: (): ReactElement => (
    <div className="flex flex-wrap items-center gap-3">
      <Button color="secondary" onPress={() => toast.success('Build succeeded', { description: 'Deployed in 2.4s.' })}>
        Success
      </Button>
      <Button color="secondary" onPress={() => toast.error('Build failed', { description: 'See the logs for details.' })}>
        Error
      </Button>
      <Button color="secondary" onPress={() => toast.info('New version available', { description: 'v0.4.1 is ready.' })}>
        Info
      </Button>
      <Button color="secondary" onPress={() => toast.warning('Unsaved changes', { description: 'Save before you leave.' })}>
        Warning
      </Button>
    </div>
  ),
    ...
    <Button
      onPress={() =>
        toast('File deleted', {
          ...
          action: { label: 'Undo', onClick: () => toast.success('Restored report.pdf') },
        })
      }
    >
      Delete file
